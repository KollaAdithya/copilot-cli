...
func (r *Repository) BuildAndPush(docker ContainerLoginBuildPusher, args *dockerengine.BuildArguments, w io.Writer) (digest string, err error) {
	if args.URI == "" {
		uri, err := r.URI()
		if err != nil {
			return "", err
	...
	if err := docker.Build(args, w); err != nil {
		return "", fmt.Errorf("build Dockerfile at %s: %w", args.Dockerfile, err)
	}

	digest, err = docker.Push(args.URI, w, args.Tags...)
	if err != nil {
