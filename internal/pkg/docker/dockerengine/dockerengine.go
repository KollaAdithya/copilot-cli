...
func (c CmdClient) Build(in *BuildArguments, writer io.Writer) error {
	dfDir := in.Context
	...
	if in.Platform != "" {
		fmt.Printf("Building your container image: docker %s\n", strings.Join(args, " "))
	}
	if err := c.runner.Run("docker", args, exec.Stdout(writer), exec.Stderr(writer)); err != nil {
...
func (c CmdClient) Push(uri string, writer io.Writer, tags ...string) (digest string, err error) {
	images := []string{}
	...
	for _, img := range images {
		if err := c.runner.Run("docker", append([]string{"push", img}, args...), exec.Stdout(writer), exec.Stderr(writer)); err != nil {
